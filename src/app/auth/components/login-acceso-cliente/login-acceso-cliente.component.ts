...
@Component({
  ...
  templateUrl: './login-acceso-cliente.component.html',
  styleUrls: ['./login-acceso-cliente.component.css']
})

/**
 * Componente de login para clientes.
 * Valida credenciales y redirige a la vista principal tras una autenticación exitosa.
 */
export class LoginAccesoClienteComponent implements OnInit, OnDestroy {
  ...
  login(): void {
    ...
    this.authService.iniciarSesionCliente(form.value).pipe(takeUntil(this.destroy$)).subscribe({
      next: () => {
